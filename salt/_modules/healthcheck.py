...
def apply_states(states=''):
  ...
  logging.debug(
      f'healthcheck_module: apply_states function caller: {calling_func}')

  if not states:
    states = ','.join(states_to_apply)

  if states: 
    logging.info(f'healthcheck_module: apply_states states: {str(states)}')
...
def docker_stop(container):
  ...
  try:
    ...
    logging.error(f'healthcheck_module: {e}')


def is_enabled():

  return bool(__salt__['pillar.get']('healthcheck:enabled', 'False'))
...
def run(checks=''):
  ...
  retval = []
  calling_func = sys._getframe().f_back.f_code.co_name
  logging.debug(f'healthcheck_module: run function caller: {calling_func}')
  ...
  logging.debug(f'healthcheck_module: run checks to be run: {str(checks)}')
  for check in checks:
    if check in allowed_functions:
      ...
      logging.warning(f'healthcheck_module: attempted to run function {check}')
...
def zeek():
  ...
  calling_func = sys._getframe().f_back.f_code.co_name
  logging.debug(f'healthcheck_module: zeek function caller: {calling_func}')
  ...
  retval = [{'zeek_restart': zeek_restart}]
